...
function UploadControl({ accept, label, onUpload }: UploadControlProps) {
  return (
    <label
...
export function UploadDrawer({ onUploadAi, onUploadAudio, onUploadReal }: UploadDrawerProps) {
  return (
    <aside className="rounded-[28px] border border-slate-800 bg-slate-900/70 p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <UploadControl accept="image/jpeg,image/png,image/webp" label="上传 AI 图片" onUpload={onUploadAi} />
        <UploadControl accept="image/jpeg,image/png,image/webp" label="上传真人图片" onUpload={onUploadReal} />
        <UploadControl accept="audio/mpeg,audio/ogg,audio/wav,audio/x-wav" label="上传音频素材" onUpload={onUploadAudio} />
      </div>
    </aside>
  );
}
